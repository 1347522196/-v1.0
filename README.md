...
这不是完整复刻商业版卖家工具，而是一个可分发、可本地运行的 `SellerSprite Lite`。
...
为了让流程更直接，这版已经移除了或弱化了这些旧设计：

- 独立的“采集任务”大面板
- 自动翻译关键词
- 前端主流程里的竞品趋势波形图
- 持久化保存 DeepSeek API Key

说明：

- 后端里还保留了部分旧 `competitor` 相关接口和表结构，主要是为了兼容旧数据，不是当前主工作流核心
...
如果你要发给别人，最稳的是把整个打包后的目录一起发过去，而不是只发单个数据库文件。
