...
class Settings:
    ...
    scraper_delay_jitter_seconds: float = float(os.getenv("AMAZON_SCRAPER_DELAY_JITTER", "0.8"))
    scraper_retry_attempts: int = int(os.getenv("AMAZON_SCRAPER_RETRY_ATTEMPTS", "3"))
    scraper_retry_backoff_seconds: float = float(
        os.getenv("AMAZON_SCRAPER_RETRY_BACKOFF", "3.0")
    )
