...
def _product_dataset(*, only_favorites: bool) -> dict[str, Any]:
    ...
    avg_price = round(mean([float(item["price"]) for item in items if item.get("price") is not None]), 2) if items else 0
    avg_rating = round(mean([float(item["rating"]) for item in items if item.get("rating") is not None]), 2) if items else 0
    avg_reviews = round(mean([int(item["review_count"]) for item in items if item.get("review_count") is not None]), 1) if items else 0
    top_items = []
