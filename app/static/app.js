      ...
      (item) => `
        <article class="favorite-card" data-id="${item.id}">
          ...
          <div class="favorite-meta">
            ...
            <span>评分 ${item.rating ?? "暂无"} / 评论 ${item.review_count ?? 0}</span>
...
async function handleAIAnalyze(event) {
  ...
  try {
    ...
    renderAIResult("AI 分析失败，请检查 API Key、网络或余额状态后重试。", "分析失败");
    setMessage(error.message, true);
  } finally {
...
async function handleKeywordRowClick(event) {
  ...
  try {
    ...
    if (button.dataset.action === "scrape-keyword") {
      ...
      setMessage(`${result.keyword} 采集完成，目标 ${result.target_products} 条，实际抓取 ${result.scraped_count} 条。`);
    }
